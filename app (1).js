...
// --- NEW: runtime reclassify & repair subskills ---
function reclassifyStandards(){
  // Merge everything, then redistribute by code prefix
  const all = [].concat(
    (STANDARDS['Reading']||[]).map(x=>({subj:'Reading',...x})),
    (STANDARDS['Math']||[]).map(x=>({subj:'Math',...x})),
    (STANDARDS['Science']||[]).map(x=>({subj:'Science',...x}))
  );
  const buckets = { Reading:[], Math:[], Science:[] };
  for(const it of all){
    const code = (it.code||'').toUpperCase();
    let dest = it.subj;
    if(code.startsWith('MA')) dest = 'Math';
    else if(code.startsWith('SC')) dest = 'Science';
    else if(code.startsWith('ELA') || code.includes('.R.')) dest = 'Reading';
    // sanitize subskills to array of strings
    let subs = Array.isArray(it.subskills) ? it.subskills : (it.subskills? [String(it.subskills)] : []);
    subs = subs.map(s=>String(s).trim()).filter(Boolean);
    // push
    buckets[dest].push({ code: it.code||'', desc: it.desc||'', subskills: Array.from(new Set(subs)) });
  }
  // Deduplicate within buckets by (code,desc)
  for(const k of Object.keys(buckets)){
    const seen = new Set(); const out=[];
    for(const x of buckets[k]){
      const key = (x.code+'||'+x.desc).toLowerCase();
...
// CSV import for Students (unchanged)
...
// --- NEW: Standards Tools (counts + CSV import override) ---
(function addStandardsTools(){
  ...
  box.innerHTML = `
    ...
    <div class="row" style="margin-bottom:8px">
      <button id="showCounts">Show subject counts</button>
      <button id="forceReclass" class="right">Force reclassify by code</button>
    </div>
    <div id="countsOut" class="note" style="white-space:pre-wrap"></div>
    <div class="row" style="margin-top:8px">
      <input type="file" id="stdCSV" accept=".csv,application/json">
      <button id="importStdBtn">Import standards (CSV/JSON)</button>
      <button id="downloadStdTemplate" class="right">Download CSV template</button>
    </div>`;
  settings.appendChild(box);

  function showCounts(){
    const cR=(STANDARDS.Reading||[]).length, cM=(STANDARDS.Math||[]).length, cS=(STANDARDS.Science||[]).length;
    document.getElementById('countsOut').textContent = `Reading: ${cR}\nMath: ${cM}\nScience: ${cS}`;
  }
  document.getElementById('showCounts').onclick = showCounts;
  document.getElementById('forceReclass').onclick = ()=>{ reclassifyStandards(); renderStandards(); showCounts(); alert('Reclassified by code prefixes (ELA→Reading, MA→Math, SC→Science).'); };

  document.getElementById('downloadStdTemplate').onclick = ()=>{
    const csv = ['Subject,Code,Description,Subskill',
      'Reading,ELA.5.R.1.1,Quote accurately from a text,Textual evidence',
      'Math,MA.5.NSO.1.1,Read/write multi-digit numbers,Expanded form',
      'Science,SC.5.N.1.1,Scientific inquiry,Plan investigations'].join('\n');
    const blob = new Blob([csv],{type:'text/csv'}); const a=document.createElement('a'); a.href=URL.createObjectURL(blob); a.download='standards_template.csv'; a.click();
  };

  document.getElementById('importStdBtn').onclick = async ()=>{
    const fi=document.getElementById('stdCSV'); const f=fi.files[0];
    if(!f) return alert('Choose a CSV or JSON file.');
    if(f.name.toLowerCase().endswith('.json')){
      const txt = await f.text();
      try{
        const obj = JSON.parse(txt);
        if(!obj.Reading||!obj.Math||!obj.Science) throw new Error('JSON must contain Reading/Math/Science arrays');
        STANDARDS = obj;
      ...
      const lines = text.split(/\r?\n/).filter(x=>x.trim().length);
      const header = lines.shift().split(',').map(s=>s.trim().toLowerCase());
      const map = { subj: header.indexOf('subject'), code: header.indexOf('code'), desc: header.indexOf('description'), sub: header.indexOf('subskill') };
      if(map.subj<0||map.code<0||map.desc<0||map.sub<0) return alert('CSV must have Subject, Code, Description, Subskill headers.');
      const buckets = { Reading:[], Math:[], Science:[] };
      for(const line of lines){
        const parts = line.split(',');
        const subj = (parts[map.subj]||'').trim();
        const code = (parts[map.code]||'').trim();
        const desc = (parts[map.desc]||'').trim();
        const sub  = (parts[map.sub]||'').trim();
        if(!subj||!code||!desc) continue;
        let S = subj.toLowerCase().includes('math') ? 'Math' : subj.toLowerCase().includes('science') ? 'Science' : 'Reading';
        if(code.toUpperCase().startsWith('MA')) S='Math'; else if(code.toUpperCase().startsWith('SC')) S='Science'; else if(code.toUpperCase().startsWith('ELA')) S='Reading';
        let entry = { code, desc, subskills: sub? [sub]: [] };
        buckets[S].push(entry);
      }
      STANDARDS = { Reading: dedupe(buckets.Reading), Math: dedupe(buckets.Math), Science: dedupe(buckets.Science) };
    }
    localStorage.setItem('kv_standards_override', JSON.stringify(STANDARDS));
    renderStandards();
    alert('Standards imported. This device now uses your uploaded standards until cleared.');
  };
})();
...
function dedupe(list){
  ...
  for(const x of list){
    ...
    if(seen.has(key)) continue;
    seen.add(key);
    x.subskills = Array.from(new Set((x.subskills||[]).map(s=>String(s).trim()).filter(Boolean)));
    out.push(x);
  }
  return out;
}
...
async function init(){
  try {
    // Use local override if present
    const over = localStorage.getItem('kv_standards_override');
    if(over){
      STANDARDS = JSON.parse(over);
    }else{
      const resp = await fetch('standards.v1271.json?v=1271');
      STANDARDS = await resp.json();
    }
  } catch (e) {
    STANDARDS = {Reading:[],Math:[],Science:[],meta:{error:String(e)}};
  }
  // Reclassify by code prefixes to fix any misfiled rows
  reclassifyStandards();
...
if('serviceWorker' in navigator){ window.addEventListener('load', ()=>navigator.serviceWorker.register('./service-worker.js?v=1270')); }
